...
import { FormulaData } from '../../types/seo';

interface FormulaSectionProps {
  data?: FormulaData;
}

export const FormulaSection: React.FC<FormulaSectionProps> = ({ data }) => {
  if (!data) return null;
    ...
    <section className="space-y-6">
      <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
        ...
        <h2 className="text-xl sm:text-2xl font-bold text-slate-900">
          {data.title || (data.type === 'math' ? 'Formula & Calculation Logic' : 'Technical Methodology')}
        </h2>
      </div>
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-6">
        ...
        {data.formulaText && (
          <div className="p-4 sm:p-6 rounded-2xl bg-slate-50 border border-slate-200 text-indigo-900 font-mono text-sm sm:text-base font-bold shadow-xs overflow-x-auto">
            <span className="block text-[10px] uppercase font-bold text-indigo-600 mb-1 tracking-wider">Formula Expression</span>
            {data.formulaText}
          </div>
        )}
        ...
        {data.variables && data.variables.length > 0 && (
          <div className="space-y-2">
            <span className="text-xs font-bold text-slate-700 uppercase tracking-wider block mb-2">Variable Definitions</span>
