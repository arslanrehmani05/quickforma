  ...
  return (
    <section className="space-y-6">
      <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
        ...
        <h2 className="text-xl sm:text-2xl font-bold text-slate-900">Related Encyclopedia Concepts</h2>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {guides.map((guide) => (
          <a
            ...
            href={guide.url}
            onClick={(e) => {
              if (onSelectView && guide.url.startsWith('/encyclopedia/')) {
                e.preventDefault();
                const slug = guide.url.replace('/encyclopedia/', '');
                onSelectView(`encyclopedia:entry:${slug}`);
              }
            }}
          ...
          >
            <div>
              <div className="flex items-center justify-between gap-2 mb-2 text-xs">
                <span className="font-semibold text-indigo-600">{guide.category}</span>
                <span className="text-[10px] text-slate-400 font-mono">Encyclopedia</span>
              </div>
